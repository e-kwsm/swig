...
if type(bdbl) is not type(BYTES):
...
if type(bout) is not type(BYTES):
...
if type(udbl) is not type_unicode_string:
...
if type(uout) is not type_unicode_string:
...
if type(bovr) is not type(BYTES):
